...
public class SkillController : MonoBehaviour
{
    ...
    public void ShowUISkill()
    {
        ...
        for (int i = 0; i < _skills.Count; i++)
        {
            ...
            {
                check1 = true;
                _view.SetUp(1, skill1.Image, skill1.Name, _skills[i].Level + 1, _skills[i].Level < 5 ? skill1.Level[_skills[i].Level + 1] : "");
            }
            if (_skills[i].Id == skill2.Id)
            {
                check2 = true;
                _view.SetUp(2, skill2.Image, skill2.Name, _skills[i].Level + 1, _skills[i].Level < 5 ? skill2.Level[_skills[i].Level + 1] : "");
            }
            if (_skills[i].Id == skill3.Id)
            {
                check3 = true;
                _view.SetUp(3, skill3.Image, skill3.Name, _skills[i].Level + 1, _skills[i].Level < 5 ? skill3.Level[_skills[i].Level + 1] : "");
            }
        }
        if (!check1)
        {
            _view.SetUp(1, skill1.Image, skill1.Name, 1, skill1.Level[1]);
        }
        if (!check2)
        {
            _view.SetUp(2, skill2.Image, skill2.Name, 1, skill2.Level[1]);
        }
        if (!check3)
        {
            _view.SetUp(3, skill3.Image, skill3.Name, 1, skill3.Level[1]);
        }
    }
